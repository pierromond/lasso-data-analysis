...
Chaque trace possède une date, mais celle-ci est exprimée sur le fuseau horaire UTC. Il est donc primordial de calculer une heure et date locale afin de pouvoir calculer correctement la répartition horaire des tags dans chaque pays ou région du monde.

Afin de connaître cette heure locale, il est possible de croiser les coordonnées GPS de chaque trace avec des polygones définissant les contours des fuseaux horaires mondiaux.

Ensuite, des données telles que le pays de la mesure ainsi que la saison, se calculent très facilement en fonction de la position GPS et de la date. Ci-dessous un exemple de fonction permettant l'acquisition d'une variable supplémentaire, ici les saisons :
...
La figure \@ref(fig:fig-road) montre la répartition d'utilisation horaire du tag "route", dans laquelle nous pouvons observer des pics à 9h00-10h00 et à 20h00-21h00.
...
Ce décalage peut être dû à plusieurs raisons que nous n'avons pas explorées : différence culturelle entre la Chine et la France, mobilité différente, etc. culturelle entre la Chine et la France, pratiques de mobilité différentes (carte à puce pour les données de transport public versus données de bruit routier), différents types de zones d'étude (métropole de Pékin et France métropolitaine). Il est toujours important de retenir que notre graphique donne la répartition de l'utilisation de tags et peut donc être influencé par le niveau d'audibilité de certains sons par exemple. Il en ressort tout de même une dynamique similaire avec les pics de circulation, qui qualifie donc au minimum ce tag comme étant pertinemment utilisé par la communauté.
...
Il semble en effet visible dans la figure \@ref(fig:fig-days) qu'une augmentation des tags "discussion" et "musique" soit amenée par la présence d'un jour férié ou par une fin de semaine. Les augmentations sur les même dates des tags tels que "végétations" et "animaux" peut amener à penser que les utilisateurs se trouvent être plus fréquemment dans des environnements naturels, tels que des parcs, lors de ces jours de détente.
...
Il est possible de voir dans la figure \@ref(fig:fig-corrplot) les corrélations entre chaque variable de notre jeu de données selon l'échelle de couleur à droite. Ainsi, pour la présence de tag "discussion" (*chatting*), il apparaît qu'une corrélation négative plutôt marquée existe entre elle et la présence de tag "route" (*road*).

Corrélation tag_chatting/tag_road :
**`r round(as.numeric((cor.test(DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(road),DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(chatting)))$estimate),3)`** ;
P-Value : 
**`r (cor.test(DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(road),DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(chatting)))$p.value`**  

Cette corrélation négative signifie une dynamique opposée entre les deux variables.
...
Corrélation tag_chatting/nombre_restaurants :
**`r round(as.numeric((cor.test(DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(rest),DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(chatting)))$estimate),3)`** ;
P-Value : 
**`r (cor.test(DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(road),DT %>% mutate(value = 1)  %>% spread(tag_name, value,  fill = 0) %>% pull(chatting)))$p.value`**  

Ces valeurs de corrélations nous permettent d'estimer statistiquement les meilleures variables prédictives si un travail de réduction de dimension était à prévoir par exemple, tout en nous donnant une idée des variables les plus déterminantes pour la création d'un modèle de prédiction de l'utilisation des tags. 
...
Pour cela, il est possible de réaliser l'apprentissage du tableau de données complète à l'aide d'un outil de machine learning appelé un *Random Forest*. Cet algorithme permet de créer un arbre de décision pour la classification d'une donnée de sortie (variable dépendante) en fonctions d'autres variables prédictives. Le Random Forest est ici utilisé pour la détermination de présence ou d'absence du tag "discussion".

Le Random Forest est entraîné sur un jeu d'entrainement correspond à 80% aléatoire du jeu de données complet, afin de pouvoir créer une matrice de confusion sur le jeu de test correspondant aux 20% restants.
Cette matrice de confusion permet d'évaluer la précision de la classification de notre modèle.
...
```{r fig-1h, fig.cap= "Cartographie de prédiction de présence du tag 'chatting' à 01h", fig.pos="H", fig.align='center', echo=FALSE, out.width="90%"}
knitr::include_graphics("plots/legend_01.png")
```
...
Il serait possible dans une future reprise des données de conserver les traces entières à la place des centroïdes des traces, à condition de modifier quelques fonctions de croisement de données géographiques. La conservation des traces entière permettrait notamment de meilleures estimations des différents environnements sonores et spatiaux traversés par l'utilisateur pendant l'enregistrement.
