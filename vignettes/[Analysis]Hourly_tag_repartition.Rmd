...
if(!exists("all_info_tempo")){
  if(!file.exists(here("data","all_info_tempo.rds"))){
    URL <- "https://zenodo.org/record/6536129/files/all_info_tempo.rds?download=1"
...
occurences <- all_info_tempo %>% dplyr::group_by(tag_name, admin, local_time = as.numeric(round(local_time))) %>% dplyr::count(name = "track_occurences")
...
ggplot(tags_hourly_repartition %>% filter(admin == "France" & tag_name == "road")) +
  ...
  scale_x_continuous(breaks = seq(from = 0, to = 23, by = 3)) +
  theme_latex(font = "lmroman", base_size = 24)
