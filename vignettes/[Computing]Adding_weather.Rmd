...
```{r setup-weather-add, include=FALSE}
...
if(!file.exists(filename)){
  ...
  RCurl::curlPerform(url = "https://donneespubliques.meteofrance.fr/donnees_libres/Txt/Synop/postesSynop.csv", writedata = f@ref)
  close(f)
}
  ...
  synopStations$distance <- apply(synopStations[,c('Longitude','Latitude')], 1,
                                  function(x){
                                    synopStations$distance <- 
                                      geosphere::distGeo(c(as.numeric(x['Longitude']),as.numeric(x['Latitude'])),c(long,lat))
                                  })
...
meteo_df <- all_info_tempo %>% filter(admin == "France")
...
meteo_france <- df_meteo %>% dplyr::mutate(pk_track = as.numeric(V1), tag_name = V2, rain_mm = as.numeric(V3), celsius = as.numeric(V4) - 273.15, wind = as.numeric(V5)*3.6, cloud = as.numeric(V6), dist_station = as.numeric(V7)) %>% dplyr::select(pk_track,rain_mm,celsius,wind)

all_info_tempo <- merge(all_info_tempo, meteo_france, by = "pk_track")
