...
if(!exists("parks")){
  if(!file.exists(here::here("raw_data","parks_france.geojson"))){
    ...
    st_write(playgrounds,here::here("raw_data","parks_france.geojson"))
  }
  else{
...
rest_fr <- st_transform(rest_fr,crs = st_crs("EPSG:2154"))
school_fr <- st_transform(school_fr,crs = st_crs("EPSG:2154"))
playgrounds <- st_transform(playgrounds,crs = st_crs("EPSG:2154"))
#Dataset "shop" contains some restaurants and bars, to not overstep on the restaurant or bars datasets which are more complete concerning restaurants, we have to deselect those
shops_fr <- st_transform(shops_fr,crs = st_crs("EPSG:2154")) %>% dplyr::filter(!type %in% unique(rest_fr$type))
sports_fr <- st_transform(sports_fr,crs = st_crs("EPSG:2154"))
tourism_fr <- st_transform(tourism_fr,crs = st_crs("EPSG:2154"))
...
pal_grid <- colorNumeric(palette = 'RdYlGn',domain = full_spatiotempo_fr$tfsd_chat)
leaflet(st_transform(full_spatiotempo_fr, crs = st_crs(4326))) %>% addProviderTiles(providers$CartoDB.Positron) %>% 
     leaflet::addCircleMarkers(color = ~pal_grid(tfsd_chat), popup = ~paste(as.character(tfsd_chat)))

pal_grid <- colorNumeric(palette = 'RdYlGn',domain = full_spatiotempo_fr$tfsd_bird)
leaflet(st_transform(full_spatiotempo_fr, crs = st_crs(4326))) %>% addProviderTiles(providers$CartoDB.Positron) %>% 
     leaflet::addCircleMarkers(color = ~pal_grid(tfsd_bird), popup = ~paste(as.character(tfsd_bird)))

pal_grid <- colorQuantile(palette = 'RdYlGn',domain = full_spatiotempo_fr$tfsd_chat / full_spatiotempo_fr$tfsd_bird)
leaflet(st_transform(full_spatiotempo_fr, crs = st_crs(4326))) %>% addProviderTiles(providers$CartoDB.Positron) %>% 
     leaflet::addCircleMarkers(color = ~pal_grid(tfsd_chat / tfsd_bird), popup = ~paste(as.character(tfsd_chat / tfsd_bird)))
...
```{r full_spatiotempo predict}

data_chat <- full_spatiotempo_fr %>% dplyr::select(-c(tfsd_chat,tfsd_bird)) %>% st_drop_geometry()

DT <- (data_chat) %>% dplyr::select(-c(contains("Ind_"),Ind,after_sunrise))

#Réaliser RF pour deviner tag chat -> faire catégorie chat_on

DT.chat <- DT %>% dplyr::mutate(chat = case_when(tag_name == "chatting" ~ TRUE, TRUE ~ FALSE)) %>% dplyr::select(-tag_name) %>% na.omit()
DT.chat$chat <- as.factor(DT.chat$chat)
DT.chat$pop_maj <- as.factor(DT.chat$pop_maj)

training_indices <- sample(nrow(DT.chat),nrow(DT.chat)*80/100)

training.cat <- DT.chat[training_indices,]
testing.cat <- DT.chat[-training_indices,]

rf.cat <- randomForest::randomForest(x = training.cat %>% dplyr::select(-(chat)), 
                                     y = training.cat$chat , data = training.cat, 
                                     xtest = testing.cat %>% dplyr::select(-(chat)), 
                                     ytest = testing.cat$chat, 
                                     importance = TRUE, ntree = 300, type = "classification", mtry = 6, keep.forest = TRUE, classwt = c(1,5))
...
DT.chat <- DT %>% na.omit() %>% group_by(pk_track) %>% mutate(tag_presence = case_when(any(tags == "chatting") ~TRUE, TRUE ~FALSE))
DT.chat$pop_maj <- as.factor(DT.chat$pop_maj)



DT.chat$tag_name <- as.factor(DT.chat$tag_name)
...
rf.cat <- randomForest::randomForest(x = training.cat %>% dplyr::select(-(tag_name)), 
                                     y = training.cat$tag_name , data = training.cat, 
                                     xtest = testing.cat %>% dplyr::select(-(tag_name)), 
                                     ytest = testing.cat$tag_name, 
                                     importance = TRUE, ntree = 300, type = "classification", mtry = 6, keep.forest = TRUE)
...
pop_vals = dplyr::select(na.omit(val_predict),contains("Ind_"))
pop_vals$maj <- colnames(pop_vals)[apply(pop_vals,1,which.max)]
DT <- na.omit(val_predict) %>% mutate(pop_maj = pop_vals$maj) %>% dplyr::select(-c(contains("Ind_"),Ind,after_sunrise))

set.seed(42)
#----

training_indices <- sample(nrow(DT),nrow(DT)*80/100)

training <- DT[training_indices,] %>% dplyr::select(-c(socialtags_score))
testing <- DT[-training_indices,] %>% dplyr::select(-c(socialtags_score))

rf <- randomForest::randomForest(chat_percent ~ .,data = training, xtest = testing %>% dplyr::select(-(chat_percent)), ytest = testing %>% dplyr::pull((chat_percent)), importance = TRUE, ntree = 300)
...
training <- DT[training_indices,] %>% dplyr::select(-c(chat_percent))
testing <- DT[-training_indices,] %>% dplyr::select(-c(chat_percent))

rf <- randomForest::randomForest(socialtags_score ~ .,data = training, xtest = testing %>% dplyr::select(-(socialtags_score)), ytest = testing %>% dplyr::pull((socialtags_score)), importance = TRUE, ntree = 300)
...
#----
DT.spatial <- DT %>% dplyr::select(-c(after_sunset))

training_indices <- sample(nrow(DT.spatial),nrow(DT.spatial)*80/100)

training.spatial <- DT.spatial[training_indices,]
testing.spatial <- DT.spatial[-training_indices,]

rf.spatial <- randomForest::randomForest(local_time ~ .,data = training.spatial, xtest = testing.spatial %>% dplyr::select(-(local_time)), ytest = testing.spatial %>% dplyr::pull((local_time)), importance = TRUE, ntree = 300)

plot(rf.spatial)
varImpPlot(rf.spatial)
rf.spatial

#----

DT.simple <- DT %>% dplyr::select(c(local_time,after_sunset,month,celsius,wind,rain_mm,Men,rest,bars,shops,schools,chat_percent))
...
rf.simple <- randomForest::randomForest(chat_percent ~ .,data = training.simple, xtest = testing.simple %>% dplyr::select(-(chat_percent)), ytest = testing.simple %>% dplyr::pull((chat_percent)), importance = TRUE, ntree = 300)
...
DT.simple <- DT %>% dplyr::select(c(local_time,after_sunset,month,celsius,wind,rain_mm,Men,rest,bars,shops,schools,socialtags_score))
...
rf.simple <- randomForest::randomForest(socialtags_score ~ .,data = training.simple, xtest = testing.simple %>% dplyr::select(-(socialtags_score)), ytest = testing.simple %>% dplyr::pull((socialtags_score)), importance = TRUE, ntree = 300, mtry = 6)
...
DT.cat <- DT %>% mutate(chat_on = case_when(
  chat_percent > 0 ~TRUE,
  chat_percent == 0 ~FALSE
)) %>% dplyr::select(-c(chat_percent,socialtags_score)) %>% dplyr::select(c(local_time,after_sunset,celsius,wind,rain_mm,Men,rest,bars,shops,schools,chat_on))
DT.cat$chat_on <- as.factor(DT.cat$chat_on) 
# %>% dplyr::select(-chat_percent)
# %>% dplyr::select(c(local_time,after_sunset,celsius,wind,Men,rest,chat_on))
#%>% dplyr::select(c(local_time,after_sunset,celsius,wind,Men,rest,bars,shops,schools,chat_on))

training_indices <- sample(nrow(DT.cat),nrow(DT.cat)*80/100)

training.cat <- DT.cat[training_indices,]
testing.cat <- DT.cat[-training_indices,]

rf.cat <- randomForest::randomForest(x = training.cat %>% dplyr::select(-(chat_on)), y = training.cat$chat_on ,data = training.cat, xtest = testing.cat %>% dplyr::select(-(chat_on)), ytest = testing.cat$chat_on, importance = TRUE, ntree = 500, type = "classification", mtry = 6, keep.forest = TRUE)
...
DT.soc <- DT %>% mutate(social_on = case_when(
  socialtags_score > 0 ~TRUE,
  socialtags_score == 0 ~FALSE
)) %>% dplyr::select(-c(chat_percent,socialtags_score))
DT.soc$social_on <- as.factor(DT.soc$social_on)
# %>% dplyr::select(-chat_percent)
# %>% dplyr::select(c(local_time,after_sunset,celsius,wind,Men,rest,chat_on))
#%>% dplyr::select(c(local_time,after_sunset,celsius,Men,rest,bars,shops,schools,chat_on))

training_indices <- sample(nrow(DT.soc),nrow(DT.soc)*80/100)

training.soc <- DT.soc[training_indices,]
testing.soc <- DT.soc[-training_indices,]

rf.soc <- randomForest::randomForest(x = training.soc %>% dplyr::select(-(social_on)), y = training.soc$social_on ,data = training.soc, xtest = testing.soc %>% dplyr::select(-(social_on)), ytest = testing.soc$social_on, importance = TRUE, ntree = 300, type = "classification", mtry = 6)
...
lon = -1.5551
hour_point = 15
date_point = "2019-07-14"
wind = 10
rain_mm = 0
celsius = 25

new_data = get_chat(lon,lat,hour_point,date_point,rain_mm,wind,celsius) %>% select(-c(lon,lat,geometry))
#prediction_chat <- get_pred(model = rf.cat, new_data = new_data)
prediction_chat <- predict(object = rf.cat, newdata = new_data, predict.all = TRUE, type = "prob")
prediction_chat$aggregate

#Validation rain----
false_rf <- c()
true_rf <- c()
for(rain in 0:20){
  new_data$rain_mm = rain
  #prediction_chat <- get_pred(model = rf.cat, new_data = new_data)
  prediction_chat <- predict(object = rf.cat, newdata = new_data, predict.all = TRUE, type = "prob")
  false_rf <- append(false_rf,prediction_chat$aggregate[1])
  true_rf <- append(true_rf,prediction_chat$aggregate[2])
}

plot(true_rf)
ggplot() + geom_smooth(aes(x = c(0:20), y = true_rf), method = "gam", formula = y ~ s(x, bs = "cs"))
...
plot(true_rf)
ggplot() + geom_smooth(aes(x = c(0:80), y = true_rf), method = "gam", formula = y ~ s(x, bs = "cs"))
...
true_rf <- c()
for(h in 0:23){
  new_data$local_time <- h
  new_data$after_sunrise
  prediction_chat <- predict(object = rf.cat, newdata = new_data, predict.all = TRUE, type = "prob")
  false_rf <- append(false_rf,prediction_chat$aggregate[1])
  true_rf <- append(true_rf,prediction_chat$aggregate[2])
}
plot(true_rf)
ggplot() + geom_smooth(aes(x = c(0:23), y = true_rf), method = "gam", formula = y ~ s(x, bs = "cs"))

#----
library(reshape2)
melted_cormat <- melt(cor(training.simple %>% dplyr::select_if(is.numeric)))
melted_cormat %>% dplyr::filter(Var1 == "chat_percent")
...
DT_ade <- DT.cat %>% dplyr::select_if(is.numeric)
res_pca <- dudi.pca(DT_ade,scale = TRUE)
...
library(ROCR)
#Faire DT.cat avec tag chat présent ou pas (catégoriel)
set.seed(42)
rfs <- randomForest::randomForest(x = DT.cat %>% dplyr::select(-(chat_on)), y = DT.cat$chat_on , importance = TRUE, ntree = 300)
pred1=predict(rfs,type = "prob")
perf = prediction(pred1[,2], DT$chat_on)
# 1. Area under curve
...
OOB.votes <- predict(rfs,x,type="prob")
...
```{r randomforest again}

#tfsd----
set.seed(42)
DT <- clean_data_fr %>% dplyr::select(-c(Ind,after_sunrise,tag_name,tfsd_bird)) %>% na.omit()

training_indices <- sample(nrow(DT),nrow(DT)*80/100)

training <- DT[training_indices,]
testing <- DT[-training_indices,]

rf <- randomForest::randomForest(tfsd_chat ~ .,data = training, xtest = testing %>% dplyr::select(-(tfsd_chat)), ytest = testing %>% dplyr::pull((tfsd_chat)), importance = TRUE, ntree = 300)
...
library(reshape2)
melted_cormat <- melt(cor(training %>% dplyr::select_if(is.numeric)))
melted_cormat %>% dplyr::filter(Var1 == "tfsd_chat")


set.seed(42)
DT <- clean_data_fr %>% dplyr::select(-c(Ind,after_sunset,tag_name,tfsd_chat)) %>% na.omit()
...
rf <- randomForest::randomForest(tfsd_bird ~ .,data = training, xtest = testing %>% dplyr::select(-(tfsd_bird)), ytest = testing %>% dplyr::pull((tfsd_bird)), importance = TRUE, ntree = 300)
...
library(reshape2)
melted_cormat <- melt(cor(training %>% dplyr::select_if(is.numeric)))
melted_cormat %>% dplyr::filter(Var1 == "tfsd_bird")

#cat_chatting----
DT.cat <- clean_data_fr %>% dplyr::select(-c(Ind,after_sunrise)) %>% na.omit()
DT.cat$tag_name <- as.factor(DT.cat$tag_name)
set.seed(42)
...
rf.cat <- randomForest::randomForest(x = training.cat %>% dplyr::select(-(tag_name)), y = training.cat$tag_name ,data = training.cat, xtest = testing.cat %>% dplyr::select(-(tag_name)), ytest = testing.cat$tag_name, importance = TRUE, ntree = 300, type = "classification", keep.forest = TRUE)
...
DT <- full_spatiotempo_fr %>% dplyr::select(-c(pleasantness)) %>% st_drop_geometry()
...
DT.catshort <- DT %>% group_by(pk_track) %>% mutate(tag_chat = case_when(any(tag_name == "chatting") ~TRUE, TRUE ~FALSE)) %>% ungroup() %>% distinct(pk_track, .keep_all = TRUE) %>%
  dplyr::select(-c(pk_track,tag_name,tfsd_bird,tfsd_chat,noise_level,after_sunrise,Ind)) %>%
  dplyr::mutate(tag_chat = as.factor(tag_chat), local_time = as.numeric(local_time), after_sunset = as.numeric(after_sunset)) %>% 
  mutate_if(is.logical,as.factor) %>% mutate(tag_chat = as.logical(tag_chat))

set.seed(42)

DTcl <- na.omit(DT.catshort) %>% dplyr::mutate(local_time = round(local_time), after_sunset = round(after_sunset), celsius = round(celsius), wind = round(wind))
...
library(rpart)
set.seed(42)
tree <- glm(formula = tag_chat ~., data = training, family = binomial)
pred <- predict(tree,testing, type = "link")
table(sign(pred), testing$tag_chat)/nrow(testing)*100

#DECISION TREE
library(rpart)
set.seed(42)
tree <- rpart(formula = tag_chat ~., data = training)
pred <- predict(tree,testing, type = "class")
table(pred, testing$tag_chat)

#RANDOMFOREST
set.seed(42)
rf.cat <- randomForest(tag_chat~., data = na.omit(training),importance = TRUE, proximity = TRUE)
pred <- predict(rf.cat,testing, type = "class")
table(pred, testing$tag_chat)/nrow(testing)*100
...
DT.catshort <- DT %>% group_by(pk_track) %>% mutate(tag_chat = case_when(any(tag_name == "chatting") ~TRUE, TRUE ~FALSE)) %>% ungroup() %>% distinct(pk_track, .keep_all = TRUE) %>% dplyr::select(-c(pk_track,tag_name,tfsd_bird,tfsd_chat,noise_level)) %>% dplyr::mutate(tag_chat = as.factor(tag_chat), local_time = as.numeric(local_time), after_sunrise = as.numeric(after_sunrise), after_sunset = as.numeric(after_sunset)) %>% mutate_if(is.logical,as.factor) %>% mutate(tag_chat = as.logical(tag_chat))

set.seed(42)

DTcl <- na.omit(DT.catshort) %>% dplyr::mutate(local_time = round(local_time), after_sunrise = round(after_sunrise), after_sunset = round(after_sunset), celsius = round(celsius), wind = round(wind))
...
RMSE <- sqrt(gbm.fit2$cv.error[min_MSE])
RMSE
...
pred <- predict(gbm.fit2,testing, type = "link")
table(sign(pred),testing$tag_chat)
...
index = sample(1:nrow(DTcl), 0.8*nrow(DTcl)) 

train = DTcl[index,] # Create the training data 

test = DTcl[-index,] # Create the test data
...
lasso_reg <- cv.glmnet(data.matrix(training), data.matrix(training$tag_chat), alpha = 1, lambda = lambdas, standardize = TRUE, nfolds = 5)
...
lasso_model <- glmnet(data.matrix(training %>% dplyr::select(-tag_chat)), data.matrix(training$tag_chat), family = binomial, alpha = 1, lambda = lambda_best, standardize = TRUE)


p <- predict(lasso_model, s = lambda_best, newx = data.matrix(testing %>% dplyr::select(-tag_chat)))

table(sign(p),testing$tag_chat)


predictions_train <- predict(lasso_model, s = lambda_best, newx = data.matrix(training))

eval_results(data.matrix(training$tag_chat), predictions_train, data.matrix(training))


predictions_test <- predict(lasso_model, s = lambda_best, newx = data.matrix(testing))

eval_results(data.matrix(testing$tag_chat), predictions_test, data.matrix(testing))
